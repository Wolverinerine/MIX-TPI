...
class Fusion(nn.Module):
    ...
    def shared_private(self, SE_embedding, PE_embedding):
        # Projecting to same sized space
        self.s_orig = s_emb = self.project_s(SE_embedding)
        self.p_orig = p_emb = self.project_p(PE_embedding)
    ...
    def forward(self, SE_embedding, PE_embedding):
        # Shared-private encoders
        self.shared_private(SE_embedding, PE_embedding)
...
class SEModel(nn.Module):
    def __init__(
        ...
        self.gating = True if gating == 1 else False
        ...
        self.tcr_intra_attn = GatingIntraSelfAttention(n_head, embedding_dim, embedding_dim, dropout=dropout_prob, gating=self.gating)
        self.pep_intra_attn = GatingIntraSelfAttention(n_head, embedding_dim, embedding_dim, dropout=dropout_prob, gating=self.gating)
        ...
        self.mlp = nn.Sequential(
            nn.Linear(
                tcr_padding_len * peptide_padding_len,
                tcr_padding_len * peptide_padding_len // 10,
            ),

            nn.ReLU(),
            nn.Linear(tcr_padding_len * peptide_padding_len // 10, 1),
            nn.Sigmoid(),
        )
    ...
    def forward(self, data: Dict):
        ...
        if merge_maps.shape[0] != 1:
            output = self.batch_norm(merge_maps.reshape(merge_maps.shape[0], -1))
        else:
            output = merge_maps.reshape(merge_maps.shape[0], -1)
...
class PEModel(nn.Module):
    def __init__(
        ...
        self.kernel_size = [[3, 3], [5, 5], [9, 9], [11, 11]]
...
class MIX_TPI(nn.Module):
    def __init__(
        ...
        self.PE_encoder = PEModel(tcr_padding_len=tcr_padding_len, peptide_padding_len=peptide_padding_len, map_num=map_num, dropout_prob=dropout_prob, hidden_channel=hidden_channel)
        self.SE_encoder = SEModel(embedding_dim=embedding_dim, tcr_padding_len=tcr_padding_len, peptide_padding_len=peptide_padding_len, dropout_prob=dropout_prob, n_head=n_head, gating=gating)

        self.Fusion = Fusion(dropout_prob, tcr_padding_len*peptide_padding_len, embedding_dim, n_head, k)

    def forward(self, data):
        PE_embedding = self.PE_encoder(data)
        SE_embedding = self.SE_encoder(data)
        output, repre_loss = self.Fusion(SE_embedding, PE_embedding)
